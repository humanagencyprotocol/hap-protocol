...
- **Resource scope on the read path** — calendar containers bind writes only today; v0.6 requires the same scope to bind reads.
- **Default-deny for undeclared read governance** — implemented for the flagship connector, not as the universal rule v0.6 states.
