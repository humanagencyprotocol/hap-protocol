...
- **`actionTypes` registry — declared by all eight profiles as of 2026-08-27** (was: undeclared everywhere). `protocol.md` → *Bounds Schema* requires every v0.5+ `boundsSchema` to declare a non-empty `actionTypes: string[]`; no published profile did. New versions now declare it, together with `appliesTo` on every `cumulative_count` bound: `calendar@0.5`, `records@0.5`, `publish@0.5`, `purchase@0.5`, `charge@0.5`, `email@0.6`, `customers@0.7`, `deploy@0.9`. Enforcement has been in place since 2026-08-26 — the AS refuses a receipt request with no `actionType` and validates registry membership where one is declared; the Gatekeeper fails closed on a write whose manifest declares none. A cross-repo test now holds the latest version of every profile to both declarations.
  <br>One registry entry is worth recording rather than discovering later: `charge@0.5` lists `unclassified` alongside `charge` and `subscribe`. The mollie connector is a **remote, vendor-controlled** MCP server whose tool list cannot be enumerated when the manifest is written and can change without notice, so anything it does not explicitly gate is treated as a write — the fail-closed choice. Those calls previously carried the label `read`, which made a receipt assert that a read had occurred; they are now labelled for what they are. Naming `unclassified` in the bound's `appliesTo` preserves enforcement exactly: the retired name-fallback's `startsWith('transaction')` case already applied that bound to every action type, so omitting it would have *loosened* a live limit while appearing to tighten one. Classifying Mollie's genuine reads needs its live tool list, which needs credentials; until then those reads are counted as consequential — over-restrictive, not unsafe.
...
### Conformance vectors — making "open protocol" checkable — targets v0.7

There is a gap between *"MIT-licensed open protocol"* and *"you can prove your
implementation conforms"*. Today the answer to "am I conformant?" is: read the
specification, implement it, and check yourself against the three signing
vectors `hap-core` publishes. Everything else — the refusals, the canonical
encodings, the error vocabulary — is prose that each implementer interprets
alone.

The existing conformance suite does not close this, and porting it would not
either. It drives *this* implementation's processes and URLs, and the
specification deliberately defines **no endpoints at all**: it fixes payloads,
error codes, canonicalization and signatures, then leaves transport to the
implementer so HAP can compose with what an operator already runs. A
conformant Authority Server may therefore share no URL with ours, and a suite
that speaks HTTP to fixed paths cannot be pointed at it.

What is portable is data. Three sets, all checkable offline with no server:

1. **Canonical bounds → hash.** A bounds object, the exact canonical string it
   must produce, and its `sha256`. A one-character disagreement here means
   nothing either party signs will ever verify against the other, and it is
   invisible until two implementations meet.
2. **Payload → signature.** An attestation and a receipt payload, a published
   test key, and the exact base64url signature. Pins field ordering, escaping,
   number formatting and encoding together.
3. **Request → required refusal.** A table of (situation, request, error code):
   no `actionType` → `INVALID_ACTION_TYPE`; over a cumulative bound →
   `CUMULATIVE_LIMIT_EXCEEDED`; review mode without an approved proposal →
   `PROPOSAL_REQUIRED`. This is the set that matters most, because refusal is
   where the safety lives, and it can be stated without reference to any URL,
   store or identity system.

**What this cannot do**, stated so it is not oversold: vectors prove agreement
on bytes and on refusals. They cannot prove *sequence* — that the receipt was
obtained **before** execution, or that cumulative state is counted correctly
across many calls. Those are claims about a running system and still need a
live suite. Vectors move the checkable surface from "read 2,000 lines and hope"
to "here are the answers" for the parts where silent disagreement is fatal;
they do not make the protocol self-certifying.

Most of the material exists: `hap-core` publishes the signing vectors, and the
reference suite already enumerates the refusals worth pinning. The work is
collecting them into an implementation-neutral, published artifact — and doing
it **before** a second implementation exists, because afterwards it is much
harder to say whose bytes are correct.
