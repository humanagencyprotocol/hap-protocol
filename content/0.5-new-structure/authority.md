...
- **Storage-key autonomy** — `boundsHash` is the wire identifier; AS storage keys MAY combine it with attester identity to disambiguate intra-group collisions.
...
- **Signing canonicalization** — explicit (RFC 8785-compatible) so independent ASes sign identical bytes.

This makes the AS a runtime dependency for execution. v0.3 ASes were touched only at attestation time and for initial key resolution. v0.4+ ASes are touched on every tool call.
...
ASes validate requests against specific Profiles. Each v0.4 Profile defines:
...
Anyone may run an AS — on a phone, server, or embedded device.
No central registry. No approval. No committee.

### Privacy-Preserving

ASes receive only the bounds (in plaintext for enforcement) and hashes for everything else:

- `bounds_hash`, `context_hash`, `execution_context_hash`
- `gate_content_hashes.intent`
- Owner DIDs and domain declarations

ASes **never** receive context content, intent text, or any other semantic content. Context content stays on the gateway, encrypted at rest.

### Per-Group Governance

In v0.4, organizational governance moves from the profile to the AS. Each group on the AS defines:
...
## Authority Server Responsibilities

A v0.4 AS has five primary responsibilities:
...
Context fields (`currency`, `action_type`) live in the contextSchema and are hashed into `context_hash` by the gateway. They are **not** part of the bounds sent to the AS — the AS only sees the context hash, and the Gatekeeper is the sole enforcer of context enum/subset constraints.

| Field | Description |
|-------|-------------|
| `profile_id` | The profile this authorization references |
| `bounds` | The full bounds object (sent in plaintext for AS enforcement) |
...
| `title` | Optional human-readable label, stored as AS metadata |
| `group_id` | Group identifier (omit or `null` for personal mode) |

The AS receives the bounds in plaintext because it must enforce them at receipt time. It does **not** receive the context content — only `context_hash`.
...
The AS MUST reject the attestation request if:
...
The Gatekeeper sends the following to the AS for every execution attempt:
...
| `idempotencyKey` | **Required on the synchronous path** (`automatic` mode, no `proposalId`). A value unique to one logical execution, generated once by the Gatekeeper and reused unchanged across retries. MUST NOT be content-derived (two intentionally identical actions are distinct executions, each counted). Rides on the request only — never in the signed receipt payload. Review-mode commits use `proposalId` instead and omit this. |

The AS derives `userId` from the authenticated request context. It is not supplied in the request body.

### AS Validation for Receipt Issuance

The AS MUST reject the receipt request if:
...
For every field in the profile's `boundsSchema.fields`, the AS looks up the field's declared `boundType` and dispatches on `boundType.kind`:
...
The AS MUST maintain cumulative state per (cumulative group, profile, actionType). The key is:
...
Cumulative state resets at calendar boundaries (daily and monthly). The AS is authoritative for cumulative state.
...
The receipt request carries both `action` (the downstream tool name, e.g., `create_payment_link`) and `actionType` (the semantic category, e.g., `charge`). The AS MUST:
...
The signature is over the canonical JSON serialization of the receipt body (excluding the `signature` field itself). See "Receipt Canonicalization" in `core.md` for the canonical serialization rules.
...
Error codes are canonical across `core.md`, `authority.md`, and `gatekeeper.md`. See the Error Codes section in `core.md` for the single authoritative list. Codes that may be returned from a receipt request include:
...
| `MALFORMED_RECEIPT_REQUEST` | Request carries a retired identifier (`attestationHash`, `frame_hash`, `path`); v0.5 uses bare `boundsHash` |
| `IDEMPOTENCY_KEY_REQUIRED` | Synchronous (`automatic`, no `proposalId`) request omitted the required `idempotencyKey` |
| `IDEMPOTENCY_MISMATCH` | `idempotencyKey` reused for a different execution (`profileId`, `action`, or `executionContext` differ) |
...
The AS maintains a revocation list. Revocation can be initiated at any time after attestation, before TTL expiry.
...
By default, the AS MUST allow revocation by:

- The original attester
- A group admin (in group mode)

The AS MAY define additional revocation policies (time windows, multi-party revocation, etc.).

### Revocation and Audit

Revocation does not invalidate the attestation cryptographically. The attestation's signature remains valid for audit purposes. Revocation only affects the AS's willingness to issue **new** receipts. Existing receipts (issued before revocation) remain valid proof of past executions.
...
An AS MAY expose endpoints that let parties other than the attester or its group verify an attestation or receipt by ID/hash. These endpoints support invariant #4 — adversarial interoperability — by letting any holder of the AS's public key check claims independently.
...
2. Third-party verification responses MUST NOT include `title`, `groupId` (when not part of the signed payload), `createdBy`, `deferredCommitmentDomains`, `intent_ciphertext`, `encrypted_keys`, or any other AS-side metadata.
...
In v0.4, organizational governance lives on the AS, configured per group.
...
If a user attempts to attest with bounds that exceed a group ceiling, the AS MUST reject the attestation request.
...
The same AS runs both modes. The same profile schemas apply to both modes. The difference is purely in the AS's enforcement of group governance.
...
The AS MUST retain the exact profile bytes for every `profile_id` it has issued attestations or receipts under, for at least as long as the longest live retention obligation against that profile. Concretely: the AS retains the profile bytes until every attestation and every receipt issued under that `profile_id` has passed its `retention_minimum` window.
...
Receipts remain cryptographically valid and retrievable after their parent attestation has expired or been revoked. The attestation's TTL and revocation status affect only the AS's willingness to issue **new** receipts against that attestation — they do not affect previously-issued receipts.
...
AS identity = its public key (e.g., `did:key:z6Mk...`).

Applications and gateways whitelist AS keys they trust. There is no global trust anchor.
...
Human (in gateway UI)
  ...
  v
Gateway (Local App)
  | (sends attestation request to AS)
  v
Authority Server
...
Gateway
  ...
  | - requests execution receipt from AS
  v
Authority Server
...
| Stateless oracle | v0.4 ASes maintain cumulative state and a revocation list. They are stateful by design. |
...
A compromised personal AS cannot issue attestations for profiles it doesn't support. Each Profile defines its own validation rules.
...
2. The gateway sends the attestation request to the AS. The AS checks `profileDomains` and sees `purchase@0.4` requires both `finance` and `compliance`.
3. The Finance Manager attests via AS — AS verifies their identity and domain authority, then signs an attestation with `resolved_domains: [{domain: "finance", did: ...}]`.
4. The Compliance Officer attests via AS — AS signs a second attestation with `resolved_domains: [{domain: "compliance", did: ...}]`.
5. Both attestations share the same `bounds_hash`, `context_hash`, and `gate_content_hashes.intent`.
6. The Gatekeeper requests a receipt from the AS. The AS validates that the union of attested domains covers the required set, then issues the receipt.
7. The agent executes the tool call.

If any required domain attestation is missing → the AS refuses to issue receipts → the agent cannot act.

---

## Summary

Authority Servers in HAP v0.4:
...
Run your own AS. Trust your own keys. Own your direction.
